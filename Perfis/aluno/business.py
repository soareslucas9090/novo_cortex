from AppCore.core.business.business import ModelInstanceBusiness
from AppCore.core.exceptions.exceptions import SystemErrorException


class AlunoBusiness(ModelInstanceBusiness):
    """
    Classe de business para operações relacionadas a Aluno.
    
    Responsável por orquestrar as operações de CRUD e
    lógica de negócio prática do modelo Aluno.
    """
    
    def atualizar_dados(self, dados):
        """
        Atualiza os dados de um aluno existente.
        
        Args:
            dados: Dicionário com os campos a serem atualizados
        
        Raises:
            SystemErrorException: Se ocorrer erro na atualização
        """
        try:
            for attr, value in dados.items():
                setattr(self.object_instance, attr, value)
            
            self.object_instance.save()
        except Exception as e:
            raise SystemErrorException('Não foi possível atualizar os dados do aluno.')

    def deletar_dados(self):
        """
        Realiza soft delete do aluno (seta ativo=False).
        
        Raises:
            SystemErrorException: Se ocorrer erro na deleção
        """
        try:
            self.object_instance.ativo = False
            self.object_instance.save()
        except Exception as e:
            raise SystemErrorException('Não foi possível deletar o aluno.')
